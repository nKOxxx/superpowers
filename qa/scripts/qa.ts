...
import { analyzeChanges, ChangeAnalysis } from './lib/analyzer.js';
import { runTests, TestResult } from './lib/test-runner.js';

type QAMode = 'targeted' | 'smoke' | 'full';
...
async function runQA(options: QAOptions, config: any): Promise<QAReport> {
  let filesChanged: string[] = [];
  let testsSelected: string[] = [];
  let results: TestResult = { passed: 0, failed: 0, skipped: 0, total: 0, details: [] };
...
function printReport(report: QAReport, duration: number): void {
  console.log('\n📊 QA Report');
  console.log('=' .repeat(40));
