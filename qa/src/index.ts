...
function detectFramework(): string | null {
  ...
  for (const [fw, config] of Object.entries(FRAMEWORKS)) {
    for (const file of config.configFiles) {
      if (existsSync(join(process.cwd(), file))) return fw;
    }
  }
...
async function runTests(options: QAOptions): Promise<{ success: boolean; output: string; coverage?: number }> {
  ...
  switch (options.mode) {
    ...
    case 'targeted':
      ...
      const testFiles = changed
        .map(mapSourceToTest)
        .filter((f): f is string => f !== null && existsSync(join(process.cwd(), f)));
...
if (import.meta.url === `file://${process.argv[1]}`) {
  program
    ...
    .action(async (opts) => {
      const result = await runTests(opts as QAOptions);
