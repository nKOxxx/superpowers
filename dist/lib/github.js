 ...
 */
export function getRepoFromGit(cwd = process.cwd()) {
    try {
 ...
 */
export async function createRelease(options, token) {
 ...
 */
export async function releaseExists(owner, repo, tag, token) {
    const octokit = createGitHubClient(token);
    try {
 ...
 */
export function createGitTag(tag, message, cwd = process.cwd()) {
    try {
        // Create annotated tag
        execSync(`git tag -a ${tag} -m "${message || `Release ${tag}`}"`, { cwd, stdio: 'inherit' });
    }
    catch (error) {
 ...
 * Push tag to remote
 */
export function pushTag(tag, cwd = process.cwd()) {
    try {
 ...
 */
export function pushCommits(branch, cwd = process.cwd()) {
    try {
 ...
 */
export function createReleaseCommit(version, files = ['package.json', 'CHANGELOG.md'], cwd = process.cwd()) {
    try {
        // Stage files
        ...
        // Create commit
        execSync(`git commit -m "chore(release): ${version}"`, { cwd });
    }
    catch (error) {
