 ...
 */
export async function runTests(command, options = {}) {
    const { cwd = process.cwd(), env = process.env, timeout = 300000, // 5 minutes
...
function parseTestOutput(output) {
    ...
    if (genericPassMatch)
        stats.passed = parseInt(genericPassMatch[1], 10);
    if (genericFailMatch)
        stats.failed = parseInt(genericFailMatch[1], 10);
 ...
 */
export async function runTestSelections(selections, baseCommand, options = {}) {
    const results = [];
 ...
 */
export function checkCoverage(results, threshold) {
    let totalCoverage = 0;
    let coverageCount = 0;
    for (const result of results) {
