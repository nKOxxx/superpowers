...
function getVerdict(scores) {
    ...
    if (scores.total >= 10 && highScores >= 2) {
        ...
        if (scores.attention >= 4)
            reasons.push('High attention potential');
        if (scores.trust >= 4)
            reasons.push('Builds significant trust');
        return { verdict: 'build', reasons };
    }
    // 6-9 stars: borderline, needs more consideration
    if (scores.total >= 6) {
        if (scores.brand >= 3)
            reasons.push('Moderate brand fit');
        if (scores.attention >= 3)
            reasons.push('Some attention potential');
        if (scores.trust >= 3)
            reasons.push('Moderate trust building');
        if (scores.brand < 3)
            reasons.push('Weak brand alignment - needs positioning work');
        if (scores.attention < 3)
            reasons.push('Low attention - distribution strategy needed');
        if (scores.trust < 3)
            reasons.push('Trust issues - credibility plan required');
        return { verdict: 'consider', reasons };
    }
    // Below 6: don't build
    if (scores.brand < 3)
        reasons.push('Poor brand fit');
    if (scores.attention < 3)
        reasons.push('Low attention capture');
    if (scores.trust < 3)
        reasons.push('Does not build trust');
...
function formatScore(value) {
    ...
    if (value >= 4)
        return chalk.green(`${value}/5 ${stars}`);
    if (value >= 3)
        return chalk.yellow(`${value}/5 ${stars}`);
    return chalk.red(`${value}/5 ${stars}`);
}
function autoScore(feature, goal) {
    ...
    for (const word of brandBoosters) {
        if (lowerFeature.includes(word) || lowerGoal.includes(word))
            brand++;
    }
