 ...
 */
export function generateRecommendation(bat, marketAnalysis, config) {
    ...
    // BAT analysis
    if (bat.brand >= 4)
        reasoning.push('Strong brand alignment');
    if (bat.attention >= 4)
        reasoning.push('High user engagement potential');
    if (bat.trust >= 4)
        reasoning.push('Builds user trust significantly');
    if (bat.brand <= 2)
        reasoning.push('Limited brand impact');
    if (bat.attention <= 2)
        reasoning.push('Low expected user engagement');
    if (bat.trust <= 2)
        reasoning.push('May erode trust');
    // Score analysis
    if (total >= 12)
        reasoning.push('Exceptional BAT score - clear winner');
    else if (total >= 10)
        reasoning.push('Good BAT score - meets threshold');
    else if (total >= 8)
        reasoning.push('Borderline score - consider carefully');
    else
        reasoning.push('Below threshold - likely not worth building');
    // Market analysis
    if (marketAnalysis) {
        if (marketAnalysis.trend === 'rising')
            reasoning.push('Aligned with market trends');
        if (marketAnalysis.trend === 'declining')
            reasoning.push('Declining market trend');
        if (marketAnalysis.riskLevel === 'high')
            reasoning.push('High implementation risk');
        if (marketAnalysis.riskLevel === 'low')
            reasoning.push('Low risk implementation');
    }
