...
function parseBrowseArgs(args) {
    const url = args[0] || '';
    const options = { url, fullPage: true };
    for (let i = 1; i < args.length; i++) {
        ...
        if (arg === '--viewport' || arg === '-v') {
            ...
            const flows = args[++i];
            if (flows)
                options.flows = flows.split(',').map(f => f.trim());
        }
...
async function captureSingleScreenshot(context, options) {
...
async function runFlows(context, options, config) {
