...
import { browse, browseToFile, VIEWPORT_PRESETS, BrowseAction } from './index.js';
...
program
  ...
  .option('-a, --action <action>', 'Action sequence (comma-separated: click:selector,type:selector:text,wait:ms,scroll:x:y,hover:selector,press:key)', '')
  .action(async (url, options) => {
    const spinner = createSpinner('Launching browser...').start();
    
    try {
      // Parse viewport
      let viewport: 'mobile' | 'tablet' | 'desktop' | Viewport = options.viewport as 'mobile' | 'tablet' | 'desktop';
        ...
        for (const part of actionParts) {
          ...
          switch (type) {
            ...
            case 'hover':
              ...
              break;
            case 'press':
              actions.push({ type: 'press', key: args[0] });
              break;
          }
        }
      }
      ...
      let result;
      if (options.output) {
        result = await browseToFile({ ...browseOptions, outputPath: options.output });
      } else {
        result = await browse(browseOptions);
      }
