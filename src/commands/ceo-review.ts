...
import type { BATScores, CEOReviewResult, Recommendation } from '../types/index.js';
...
export function ceoReviewCommand(program: Command): void {
  program
    .command('ceo-review')
    ...
    .action(async (options: CEOReviewOptions) => {
      try {
...
async function runCEOReview(options: CEOReviewOptions): Promise<void> {
...
function calculateScores(options: CEOReviewOptions): BATScores {
  const scores: BATScores = { brand: 3, attention: 3, trust: 3 };
  ...
  // Brand scoring
  if (feature.includes('ai') || feature.includes('ml')) scores.brand += 1;
  if (goal.includes('differentiate') || goal.includes('innovation')) scores.brand += 1;
  if (feature.includes('unique') || feature.includes('first')) scores.brand += 1;
  if (feature.includes('basic') || feature.includes('simple')) scores.brand -= 1;
  if (feature.includes('me too') || feature.includes('copy')) scores.brand -= 2;

  // Attention scoring
  if (goal.includes('daily') || goal.includes('core')) scores.attention += 1;
  if (audience.includes('all') || audience.includes('everyone')) scores.attention += 1;
  if (goal.includes('engagement') || goal.includes('retention')) scores.attention += 1;
  if (feature.includes('admin') || feature.includes('settings')) scores.attention -= 1;
  if (goal.includes('rarely') || goal.includes('occasionally')) scores.attention -= 1;

  // Trust scoring
  if (feature.includes('security') || feature.includes('privacy')) scores.trust += 2;
  if (feature.includes('backup') || feature.includes('export')) scores.trust += 1;
  if (options.trust?.includes('certified') || options.trust?.includes('audited')) scores.trust += 1;
  if (feature.includes('beta') || feature.includes('experimental')) scores.trust -= 1;
...
function generateReview(
  ...
  scores: BATScores,
  config: ReturnType<typeof mergeWithDefaults>['ceoReview']
): CEOReviewResult {
...
function displayResults(result: CEOReviewResult): void {
