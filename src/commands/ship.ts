...
export const shipCommand = new Command('ship')
  ...
  .argument('[version]', 'Version type or explicit version', 'patch')
  .option('-d, --dry-run', 'Preview only')
  .option('--skip-changelog', 'Skip changelog')
  .option('--skip-git', 'Skip git')
  .option('--skip-github', 'Skip GitHub')
  .action((v, opts) => ship(v, opts.dryRun, opts.skipChangelog, opts.skipGit, opts.skipGithub));

shipCommand.command('preview')
  .action(() => {
    const commits = getCommits();
...
shipCommand.command('version')
  .action(() => console.log(getVersion()));
