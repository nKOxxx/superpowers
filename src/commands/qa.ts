...
export const qaCommand = new Command('qa')
  ...
  .option('-m, --mode <mode>', 'Mode: targeted, smoke, full', 'targeted')
  .option('-c, --coverage', 'Enable coverage')
  .action((opts) => runQAMode(opts.mode, opts.coverage));

qaCommand.command('targeted')
  .option('-c, --coverage', 'Enable coverage')
  .action((opts) => runQAMode('targeted', opts.coverage));

qaCommand.command('smoke')
  .option('-c, --coverage', 'Enable coverage')
  .action((opts) => runQAMode('smoke', opts.coverage));

qaCommand.command('full')
  .option('-c, --coverage', 'Enable coverage')
  .action((opts) => runQAMode('full', opts.coverage));

qaCommand.command('frameworks')
  .action(() => {
    const fw = detectFramework();
