...
export class PlanCEOReviewSkill {
  private calculateAutoScores(feature: string, description?: string): BATScores {
  ...
  private getRecommendation(totalScore: number): { recommendation: CEORReviewResult['recommendation']; reasoning: string } {
    if (totalScore >= 10) {
      return {
  ...
  private generateNextSteps(recommendation: CEORReviewResult['recommendation'], scores: BATScores): string[] {
    const steps: string[] = [];
    ...
    switch (recommendation) {
      ...
      case 'dont-build':
        ...
        if (scores.brand < 3) steps.push('💡 Consider how to strengthen brand alignment');
        if (scores.attention < 3) steps.push('💡 Explore ways to increase attention/virality');
        if (scores.trust < 3) steps.push('💡 Identify trust-building opportunities');
        break;
    }
  ...
  async review(options: CEORReviewOptions): Promise<CEORReviewResult> {
...
if (require.main === module) {
  async function main() {
    ...
    const options: CEORReviewOptions = { feature };
