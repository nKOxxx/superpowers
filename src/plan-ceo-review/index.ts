...
program.name('plan-ceo-review').description('Product strategy review').version('1.0.0');
program.argument('<feature>', 'Feature name').argument('<goal>', 'Feature goal')
  ...
  .option('--json', 'JSON output').option('-o, --output <path>', 'Save to file')
  .action((feature, goal, opts) => review({ feature, goal, marketSize: opts.marketSize, urgency: opts.urgency, resources: opts.resources }, opts.json, opts.output));
program.parse();