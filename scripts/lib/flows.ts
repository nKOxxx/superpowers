...
async function executeStep(
  ...
  switch (step.action) {
    ...
    case 'type': {
      ...
      if (step.value === undefined) throw new Error('Type action requires value');
      await page.locator(step.target).fill(step.value);
      break;
    }

    case 'wait': {
      const delay = step.options?.ms as number || 1000;
    ...
    case 'screenshot': {
      ...
      await page.screenshot({ path, fullPage: step.options?.fullPage as boolean ?? true });
 ...
 */
export function createFlowFromPaths(
  name: string,
  baseUrl: string,
  paths: string[],
 ...
 */
export function formatFlowResults(results: FlowResult[]): string {
  const lines: string[] = ['Flow Execution Results', '======================'];
