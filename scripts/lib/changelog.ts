...
export function generateChangelogEntry(
  ...
  for (const commit of commits) {
    ...
    if (parsed) {
      changes.push({
        type: parsed.type as ChangelogEntry['changes'][0]['type'],
        scope: parsed.scope,
...
export function formatChangelogEntry(entry: ChangelogEntry): string {
  ...
  const grouped = entry.changes.reduce((acc, change) => {
    ...
    return acc;
  }, {} as Record<string, ChangelogEntry['changes']>);
 ...
 */
export function readChangelog(cwd: string = process.cwd()): string {
 ...
 * Get the last tag
 */
export function getLastTag(): string | undefined {
  try {
    const output = execSync(
