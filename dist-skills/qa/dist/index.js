...
function parseArgs() {
    ...
    const options = { mode: 'targeted', coverage: false };
...
function runTests(framework, options) {
    const result = { passed: 0, failed: 0, skipped: 0, duration: '0s', failures: [] };
    let cmd = '';
    if (framework === 'vitest') {
        cmd = 'npx vitest run';
        if (options.coverage)
        ...
        if (options.mode === 'smoke')
            cmd += ' --reporter=verbose --testNamePattern="smoke|basic"';
    }
        ...
        if (options.coverage)
            cmd += ' --coverage';
        if (options.mode === 'smoke')
            cmd += ' --testNamePattern="smoke|basic"';
    }
