...
function parseArgs(args: string[]): { question: string; interactive: boolean } {
  const result = {
    question: '',
...
function autoScore(question: string): BATAnalysis {
  ...
  if (q.includes('free') || q.includes('save money')) attentionScore = 4;
  ...
  if (q.includes('security') || q.includes('privacy')) trustScore = 4;
...
export function handleCallback(callbackData: string, currentState: any): SkillResult {
  ...
  const dimension = parts[1] as 'brand' | 'attention' | 'trust';
  const score = parseInt(parts[2], 10) as BATScore;
  ...
  const dimensions: Array<'brand' | 'attention' | 'trust'> = ['brand', 'attention', 'trust'];
