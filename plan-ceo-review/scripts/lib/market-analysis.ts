...
export async function analyzeMarket(options: FeatureOptions): Promise<MarketAnalysis> {
...
function identifyCompetitors(description: string, market: string): string[] {
...
function analyzeTrend(description: string, market: string): 'rising' | 'stable' | 'declining' | 'unknown' {
  ...
  for (const kw of risingKeywords) {
    if (description.includes(kw)) return 'rising';
  }
  
  for (const kw of decliningKeywords) {
    if (description.includes(kw)) return 'declining';
  }
  
  return 'stable';
}

function assessRisk(description: string, market: string): 'low' | 'medium' | 'high' | 'unknown' {
  ...
  for (const kw of highRiskKeywords) {
    if (description.includes(kw)) return 'high';
  }
  
  for (const kw of lowRiskKeywords) {
    if (description.includes(kw)) return 'low';
  }
  
  return 'medium';
}

function assessTiming(description: string): string {
  const urgentKeywords = ['security', 'bug', 'fix', 'critical', 'vulnerability'];
  
  for (const kw of urgentKeywords) {
    if (description.includes(kw)) return 'Urgent - security/critical issue';
  }
