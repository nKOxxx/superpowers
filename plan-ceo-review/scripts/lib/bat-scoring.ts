...
export async function evaluateBAT(options: FeatureOptions): Promise<BATEvaluation> {
...
function calculateScores(options: FeatureOptions): BATScores {
  ...
  if (description.includes('brand') || description.includes('identity')) brand = 5;
  else if (description.includes('ui') || description.includes('design')) brand = 4;
  else if (description.includes('api') || description.includes('internal')) brand = 2;
  else if (description.includes('fix') || description.includes('bug')) brand = 2;
  
  // Attention scoring (0-5)
  let attention = 3; // Default neutral
  if (goal.includes('engagement') || goal.includes('daily')) attention = 5;
  else if (goal.includes('retention') || goal.includes('usage')) attention = 4;
  else if (description.includes('notification') || description.includes('alert')) attention = 4;
  else if (description.includes('admin') || description.includes('settings')) attention = 2;
  else if (description.includes('analytics') || description.includes('report')) attention = 3;
  
  // Trust scoring (0-5)
  let trust = 3; // Default neutral
  if (description.includes('security') || description.includes('auth')) trust = 5;
  else if (description.includes('privacy') || description.includes('gdpr')) trust = 5;
  else if (description.includes('payment') || description.includes('billing')) trust = 4;
  else if (description.includes('backup') || description.includes('export')) trust = 4;
  else if (description.includes('experimental') || description.includes('beta')) trust = 2;
...
function generateRationale(options: FeatureOptions, scores: BATScores): BATRationale {
  const description = options.description || options.feature || '';
  
  const brandReasons: Record<number, string> = {
    0: 'No brand impact',
  ...
  const attentionReasons: Record<number, string> = {
