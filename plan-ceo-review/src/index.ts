...
export function calculateRecommendation(totalScore: number): 'BUILD' | 'CONSIDER' | "DON'T BUILD" {
  if (totalScore >= 12) return 'BUILD';
  if (totalScore >= 8) return 'CONSIDER';
  return "DON'T BUILD";
}

export function generateReasoning(scores: BATScores, totalScore: number): string {
  const parts: string[] = [];
...
export function generateNextSteps(recommendation: 'BUILD' | 'CONSIDER' | "DON'T BUILD", feature: string): string[] {
  const steps: string[] = [];
...
export function ceoReview(options: CEOReviewOptions): CEOReviewResult {
