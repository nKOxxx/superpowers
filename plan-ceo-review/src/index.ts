...
function autoScore(idea: string): BATScore {
  ...
  for (const word of BRAND_INDICATORS.positive) {
    if (lowerIdea.includes(word)) brand += 0.5;
  }
  ...
  for (const word of ATTENTION_INDICATORS.positive) {
    if (lowerIdea.includes(word)) attention += 0.5;
  }
  ...
  for (const word of TRUST_INDICATORS.positive) {
    if (lowerIdea.includes(word)) trust += 0.5;
  }
...
function calculateStars(scores: BATScore): number {
  const total = scores.brand + scores.attention + scores.trust;
  // Convert to 10-star scale
  return Math.round((total / 15) * 10);
}

function generateRecommendation(analysis: BATAnalysis): Recommendation {
  ...
  let decision: 'BUILD' | 'CONSIDER' | 'DONT_BUILD';
  let confidence: 'HIGH' | 'MEDIUM' | 'LOW';
  
  if (stars >= 8) {
    decision = 'BUILD';
    confidence = 'HIGH';
    if (reasoning.length === 0) reasoning.push('Exceptional BAT score - strong across all dimensions');
  } else if (stars >= 6) {
    decision = 'CONSIDER';
    confidence = 'MEDIUM';
...
export function analyzeBAT(options: BATOptions): BATAnalysis {
  ...
  if (options.brand) scores.brand = options.brand;
  if (options.attention) scores.attention = options.attention;
  if (options.trust) scores.trust = options.trust;
