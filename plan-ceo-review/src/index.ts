...
function calculateRecommendation(totalScore: number): { recommendation: Recommendation; stars: string } {
...
function calculateConfidence(bat: BATAnalysis): Confidence {
...
function generateRisks(bat: BATAnalysis, question: string): string[] {
  const risks: string[] = [];
...
function generateNextSteps(recommendation: Recommendation, bat: BATAnalysis): string[] {
  const steps: string[] = [];
...
function autoScore(question: string): BATAnalysis {
  const q = question.toLowerCase();
  
  // Brand scoring heuristics
  let brandScore: BATScore = 3;
  if (q.includes('ai') || q.includes('ml') || q.includes('machine learning')) brandScore = 4;
  if (q.includes('mobile app') || q.includes('ios') || q.includes('android')) brandScore = 3;
  if (q.includes('new market') || q.includes('expansion')) brandScore = 3;
  if (q.includes('premium') || q.includes('enterprise')) brandScore = 4;
  if (q.includes('core') || q.includes('flagship')) brandScore = 5;
  if (q.includes('unrelated') || q.includes('side project')) brandScore = 2;
  
  // Attention scoring heuristics
  let attentionScore: BATScore = 3;
  if (q.includes('ai') || q.includes('automation')) attentionScore = 4;
  if (q.includes('free') || q.includes('save money') || q.includes('discount')) attentionScore = 4;
  if (q.includes('integration') || q.includes('connect')) attentionScore = 3;
  if (q.includes('export') || q.includes('report')) attentionScore = 2;
  if (q.includes('viral') || q.includes('social')) attentionScore = 4;
  if (q.includes('enterprise') || q.includes('b2b')) attentionScore = 3;
  
  // Trust scoring heuristics
  let trustScore: BATScore = 3;
  if (q.includes('security') || q.includes('privacy') || q.includes('encryption')) trustScore = 4;
  if (q.includes('beta') || q.includes('experimental')) trustScore = 2;
  if (q.includes('partner') || q.includes('integration')) trustScore = 3;
  if (q.includes('proven') || q.includes('industry standard')) trustScore = 4;
  if (q.includes('new technology') || q.includes('cutting edge')) trustScore = 2;
  if (q.includes('core competency')) trustScore = 5;
...
export function review(options: ReviewOptions): CEOReviewResult {
  let bat: BATAnalysis;
...
function formatResult(result: CEOReviewResult): string {
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    if (arg.startsWith('--')) {
      ...
      switch (key) {
        case 'brand':
        case 'b':
          options.brand = parseInt(value || args[++i], 10) as BATScore;
          options.autoScore = false;
          break;
        case 'attention':
        case 'a':
          options.attention = parseInt(value || args[++i], 10) as BATScore;
          options.autoScore = false;
          break;
        case 'trust':
        case 't':
          options.trust = parseInt(value || args[++i], 10) as BATScore;
          options.autoScore = false;
          break;
        case 'auto-score':
