...
export class PlanCeoReviewSkill {
  ...
  review(feature: string, options: ReviewOptions = {}): ReviewResult {
  ...
  compare(feature1: string, feature2: string, options: ReviewOptions = {}): CompareResult {
  ...
  private calculateStarRating(feature: string, options: ReviewOptions): StarRating {
    const featureLower = feature.toLowerCase();
  ...
  private getRecommendation(batTotal: number, stars: number): string {
  ...
  private generateNextSteps(batScore: BatScore, starRating: StarRating): string[] {
    const steps: string[] = [];
  ...
  private estimateResources(starRating: StarRating): string {
  ...
  private estimateTimeline(starRating: StarRating): string {
    if (starRating.feasibility >= 8) {
  ...
  formatReview(result: ReviewResult, format: 'text' | 'json' | 'markdown' = 'text'): string {
    switch (format) {
      case 'json':
  ...
  private formatText(result: ReviewResult): string {
    const batBars = {
