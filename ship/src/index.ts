...
function parseConventionalCommit(message: string): CommitInfo['type'] {
  ...
  for (const [type, pattern] of Object.entries(patterns)) {
    if (pattern.test(message)) return type as CommitInfo['type'];
  }
  return 'other';
}
...
function getCommits(sinceTag?: string): CommitInfo[] {
  try {
...
function generateChangelog(commits: CommitInfo[], newVersion: string): string {
  ...
  for (const commit of commits) {
    switch (commit.type) {
      ...
      case 'style': sections['Styles'].push(commit); break;
      case 'refactor': sections['Code Refactoring'].push(commit); break;
      case 'test': sections['Tests'].push(commit); break;
      case 'chore': sections['Chores'].push(commit); break;
...
async function updateChangelog(newContent: string, dryRun: boolean): Promise<void> {
...
async function updatePackageVersion(version: string, dryRun: boolean): Promise<void> {
  const pkgPath = join(process.cwd(), 'package.json');
...
async function createGitTag(version: string, message: string | undefined, dryRun: boolean): Promise<void> {
...
export async function ship(options: ShipOptions): Promise<{ success: boolean; version: string; changelog?: string }> {
  try {
