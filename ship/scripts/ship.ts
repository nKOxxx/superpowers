...
import { bumpVersion, VersionType } from './lib/version.js';
...
function parseShipArgs(args: string[]): ShipOptions {
  const options: Partial<ShipOptions> = {};
  ...
  return options as ShipOptions;
}

async function runShip(options: ShipOptions, config: any): Promise<ReleaseResult> {
  // Validation checks
  if (!options.dryRun) {
