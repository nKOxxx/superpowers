...
function getCommitsSinceLastTag(): Commit[] {
  try {
    const logOutput = execSync(
      'git log $(git describe --tags --abbrev=0 2>/dev/null || echo HEAD~20)..HEAD --format="%H|%s|%b---END---"',
      { encoding: 'utf-8' }
    );
...
function parseCommitMessage(subject: string): Partial<Commit> {
...
function categorizeCommits(commits: Commit[], preset: ChangelogPreset): CategorizedCommits {
  const sections: CategorizedCommits = {
    features: [],
    fixes: [],
    docs: [],
    style: [],
