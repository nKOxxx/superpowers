...
async function parseActions(actionsStr?: string): Promise<Action[]> {
  if (!actionsStr) return [];
  
  try {
    return JSON.parse(actionsStr) as Action[];
  } catch {
...
export async function browse(options: BrowseOptions): Promise<{ path?: string; base64?: string; success: boolean; error?: string }> {
